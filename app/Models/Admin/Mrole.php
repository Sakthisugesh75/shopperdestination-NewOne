...
class Mrole extends Model
{
  ...
  public function getRoleListDt($postData=null){
    $totalData =DB::table('user_role')->where('status',"1")->count();
  ...
  public function updateRole($id,$role_name,$updated_by){
    ...
    if(empty($item)){
      ...
      $this->role->where('user_role.id', $id);
  ...
  public function getAllRole(){
    $this->role->select('*')->where('status',"1");
    $this->role->orderBy('id', 'DESC');
    $this->role->limit(1000);
    $item=$this->role->get()->toArray();
    return $item;
  }
