...
class Category extends Controller
{
  ...
  function getallCategory(){
      $list=$this->mcategory->getallCategory();
      if(!empty($list)){
  ...
  function createCategory(){
      ...
      if($name != "" && $slug != ""){
        $i_status=$this->mcategory->createCategory($name,$slug,$sortdescription,$fulldescription,$group_tag,$created_by);
        if($i_status=='A'){
          $this->_response_data['status']="FAILED";
          ...
          $this->_response_data['code']="105";
            ...
            $file_name = md5("category|media|" . date('Y-m-d H:i:s'));
            $target_dir = "cdn/category/o/".$id;
            $return = $this->helper->saveCdn($media_ext, $media_tmp_name, $media_error, $file_name, $target_dir);
            if ($return['status'] == 'S') {
            $m_target_dir = "cdn/category/m/".$id;
            $this->helper->imageCrop($return['url'], $m_target_dir, $file_name, $media_ext, 420, 600,$image_url,'#ffffff'); //resize the image
            ...
            if (!empty($image_url)  ) {
              $this->mcategory->imageupload($image_url, $id);
              }
          }
          $this->_response_data['status']="SUCCESS";
  ...
  function updateCategory(){
      ...
      if($id != "" && $name != "" && $slug != ""){
        $i_status=$this->mcategory->updateCategory($id,$name,$slug,$sortdescription,$fulldescription,$group_tag,$updated_by);
        if($i_status){
            $image_url = '';
            if (isset($_FILES['media']) && !empty($_FILES['media']) ) {
            $media_name = $_FILES['media']['name'];
