...
class City extends Controller
{
  ...
  $this->helper = new Helper();
    ...
    $res = explode("@",$url);
    $method = $res[1];
    if(!in_array($method, array('register','login','getCityByState'))){
  ...
  function listCitygt(){
      ...
      $list=$this->mcity->getCityListGt($role, $status, $limit, $offset);
      $total_size=0;
      $record=array();
  ...
  function listCitydt(){
    ...
    $data = $this->mcity->getCityListDt($postData);
    return $data;
  }

  function listCityById (){
    $id=$this->request->input('id');
    if($id){
        $list=$this->mcity->getCityListById($id);
        $record=array();
        if(!empty($list) ){
            $record=$list;
        }
...
function getCityByState(){
  $state_id=$this->request->input('id');
  if($state_id != ""){
      $list=$this->mcity->getcitybystate($state_id);
      if(!empty($list)){
        $this->_response_data['status']="SUCCESS";
        $this->_response_data['list']=$list;
        $this->_response_data['code']="206";
  ...
  function createCity(){
      ...
      $city_name=$this->request->input('city_name');
			$state_id=$this->request->input('state_id');
			$country_id=$this->request->input('country_id');
			$created_by=$this->_user['user_id'];
      if($city_name != ""){
        $i_status=$this->mcity->createCity($city_name,$state_id,$country_id,$created_by);
        if($i_status=='A'){
          $this->_response_data['status']="FAILED";
  ...
  function updateCity(){
      ...
      if($city_id != ""){
        $i_status=$this->mcity->updateCity($city_id,$city_name,$state_id,$country_id,$updated_by);
        if($i_status){
          $this->_response_data['status']="SUCCESS";
  ...
  function deleteCity(){
        $id=$this->request->input('id');
        if($id){
        $status=$this->mcity->deleteCity($id);
        if($status){
          $this->_response_data['status']="SUCCESS";
