...
class Groupcode extends Controller
{
  ...
  function getallGroupcode(){
      $list=$this->mgroupcode->getallGroupcode();
      if(!empty($list)){
  ...
  function listGroupcodedt(){
    $postData = $this->request->input();
    $data = $this->mgroupcode->getGroupcodeListDt($postData);
    return $data;
  }

  function listGroupcodeById (){
    $id=$this->request->input('id');
    if($id){
        $list=$this->mgroupcode->getGroupcodeListById($id);
        if(!empty($list)){
          $this->_response_data['status']="SUCCESS";
          $this->_response_data['list']=$list;
          $this->_response_data['code']="206";
  ...
  function createGroupcode(){
      ...
      $this->checkValidator($rules);
			...
			$name=$this->request->input('product_name');
			$group_code=$this->request->input('group_code');

			$created_by=$this->_user['user_id'];
      if($name != "" ){
        $i_status=$this->mgroupcode->createGroupcode($name,$group_code,$created_by);
        if($i_status=='A'){
          $this->_response_data['status']="FAILED";
  ...
  function updateGroupcode(){
      ...
      $name=$this->request->input('u_product_name');
      $group_code=$this->request->input('u_group_code');

			$updated_by=$this->_user['user_id'];
      if($id != "" && $name != "" ){
        $i_status=$this->mgroupcode->updateGroupcode($id,$name,$group_code,$updated_by);
        if($i_status){
  ...
  function deleteGroupcode(){
        $id=$this->request->input('id');
        if($id){
        $status=$this->mgroupcode->deleteGroupcode($id);
        if($status){
          $this->_response_data['status']="SUCCESS";
          ...
          $this->_response_data['status']="FAILED";
          $this->_response_data['code']="108";
          $this->_response_data['message']="Please remove Product with This Group Code";
        }
      }else{
        $this->_response_data['status']="FAILED";
        $this->_response_data['code']="109";
