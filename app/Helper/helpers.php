...
function resizeImage($imagePath, $maxWidth, $maxHeight, $ext) {
    if (!file_exists($imagePath)) return 0;

    // Get original image size
    list($originalWidth, $originalHeight) = getimagesize($imagePath);

    // No resize needed
    if ($originalWidth <= $maxWidth && $originalHeight <= $maxHeight) return 1;
    ...
    // Create destination image with true color
    ...
    switch (strtolower($ext)) {
        case '.jpg':
        case '.jpeg':
            imagejpeg($dstImage, $imagePath, 95); // High quality
            break;
        case '.png':
            imagepng($dstImage, $imagePath, 2); // Compression level 0–9
            break;
        case '.gif':
