    ...
    <div class="breadcrumb-wrapper breadcrumb-wrapper-2 breadcrumb-contacts">
        ...
        <div>
            <button type="button" class="btn btn-primary" data-bs-toggle="modal"
                data-bs-target="#addUser"> Add Color
            </button>
        </div>
                ...
                <div class="ec-cat-form">
                    ...
                    <form action="#" id="addform" method="POST">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-12">
                                <input id="color" name="color" name="text" class="form-control here slug-title" type="text">
                            </div>
                        </div>
                        <div class="form-group row">
                            <label for="text" class="col-12 col-form-label">Color Code</label>
                            <div class="col-12">
                                <input id="color_code" name="color_code"  class="form-control " type="color">
                            </div>
                        </div>
                   ...
                   <form action="#" id="form" method="POST">
                    ...
                    <input type="hidden" name="u_id" id="u_id">
                       ...
                       <div class="form-group row">
                           <label for="text" class="col-12 col-form-label">Color</label>
                           <div class="col-12">
                               <input id="u_color" name="u_color"  class="form-control " type="text">
                           </div>
                       </div>
                       ...
                       <div class="form-group row">
                        <label for="text" class="col-12 col-form-label">Color Code</label>
                        <div class="col-12">
                            <input id="u_color_code" name="u_color_code"  class="form-control " type="color">
                        </div>
                    </div>
    ...
    table.destroy();
	   	$('#responsive-data-tablec').DataTable({
            ...
            "responsive": true,
	      	'serverMethod': 'GET',
	      	'ajax': {
	          'url':  '<?php echo url('/');?>/api/v1/color/list-colordt'
	      	},
            "columns": [
                { "data": 'id' },
                { "data": 'color' },
                {  "render": function( data, type, row, meta ) {
                                           var c = `<span style="color: ${row.color_code}" width="20" height="20">■</span>`;
...
//  $("#save_button_loading").show();
 ...
 var url = '<?php echo url('/');?>/api/v1/color/create-color';
 var fetchOptions = {
   method: 'POST',
   headers,
   body: formData
        ...
        $("#save_button_loading").hide();
           if (data.status == 'SUCCESS') {
            ...
            if (result.isConfirmed) {
                window.location.href = "<?=url('/');?>/manage-color";
            }})
...
e.preventDefault();
    ...
    $.ajax({
                type:'GET',
                url:'<?php echo url('/');?>/api/v1/color/list-color-ById',
                data:{'id': id},
                success:function(data) {
                    ...
                    if(data.status =="SUCCESS"){
                        ...
                        $('#u_color').val(data.list.color);
                        $('#u_color_code').val(data.list.color_code);
...
var formInstance = document.getElementById('form');
 ...
 var url = '<?php echo url('/');?>/api/v1/color/update-color';
 var fetchOptions = {
   method: 'POST',
   headers,
   body: formData
 ...
 event.preventDefault();
        ...
        Swal.fire({
            ...
            $.ajax({
                        type:'DELETE',
                        url:'<?php echo url('/');?>/api/v1/color/delete-color',
                        data:{'id': id},
                        success:function(data) {
                            console.log(data);
                            if(data.status =="SUCCESS")
