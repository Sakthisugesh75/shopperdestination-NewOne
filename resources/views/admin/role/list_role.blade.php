    ...
    <div class="breadcrumb-wrapper breadcrumb-contacts">
        <div>
            ...
            <p class="breadcrumbs"><span><a href="index.html">Home</a></span>
                <span><i class="mdi mdi-chevron-right"></i></span>User Role
            </p>
        </div>
        <div>
            <button type="button" class="btn btn-primary" data-bs-toggle="modal"
                data-bs-target="#addUser"> Add Role
            </button>
        </div>
    </div>
    <div class="row">
      ...
      <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
          <div class="modal-content">
              <form action="#" id="form" method="POST">
                  ...
                  <div class="modal-body px-4">
                    <input type="hidden" class="form-control" id="u_id" name="u_id" value="">
                      ...
                      <div class="row mb-2">
                          <div class="col-lg-10">
                              <div class="form-group">
                                  <label for="role_name">Role name</label>
                                  <input type="text" class="form-control" id="u_role_name" name="u_role_name" value="" required>
                              </div>
                          </div>
...
<script>
    ...
    table.destroy();
	   	$('#responsive-data-table1').DataTable({
            ...
            "responsive": true,
	      	'serverMethod': 'GET',
	      	'ajax': {
	          'url':  '<?php echo url('/');?>/api/v1/role/list-roledt'
	      	},
            "columns": [
	         	{ "data": 'id' },
                { "data": 'role_name' },
    ...
    $(document).on("click", ".delete-value", function (e) {
        ...
        Swal.fire({
            ...
            $.ajax({
                        type:'DELETE',
                        url:'<?php echo url('/');?>/api/v1/role/delete-role',
                        data:{'id': id},
...
// insertform.addEventListener('submit', function(event) {
 ...
 var url = '<?php echo url('/');?>/api/v1/role/create-role';
 var fetchOptions = {
   method: 'POST',
   headers,
   body: formData
       ...
       .then(data => {
        ...
        $("#save_button_loading").hide();
           if (data.status == 'SUCCESS') {
            ...
            if (result.isConfirmed) {
                window.location.href = "<?=url('/');?>/manage-role";
            }})
           } else {
               Swal.fire(
               'Failed!',
...
// updateform.addEventListener('submit', function(event) {
 ...
 var url = '<?php echo url('/');?>/api/v1/role/update-role';
 var fetchOptions = {
   method: 'POST',
   headers,
   body: formData
...
e.preventDefault();
    ...
    $.ajax({
                type:'GET',
                url:'<?php echo url('/');?>/api/v1/role/get-roledt',
                data:{'id': id},
                success:function(data) {
                    console.log(data);
                    if(data.status =="SUCCESS"){
